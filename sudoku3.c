...
ll getPossibleValues(int** input, int row, int column){
	...
	for(i = 0; i < SIZE; i++){
		res |= ((1LL)<<i);
		if(input[row][i] != 0){
			// res &= ~((1LL)<<(input[row][i]-1));
			n_res |= ((1LL)<<(input[row][i]-1));
		}
		if(input[i][column] != 0){
			// res &= ~((1LL)<<(input[i][column] - 1));
			n_res |= ((1LL)<<(input[i][column]-1));
		}
	}
	int j;
	...
	for(i = 0; i < MINIGRIDSIZE; i++){
		for(j = 0; j < MINIGRIDSIZE; j++){
			if(input[mini_row + i][mini_column + j] != 0){
				// res &= ~((1LL)<<(input[mini_row+i][mini_column+j] - 1));
				n_res |= ((1LL)<<(input[mini_row+i][mini_column+j] - 1));
			}
		}
	}
	res = res&(~n_res);
...
int updatePossibleGrid(int** input, int r_num, int c_num, ll** possibleGrid){
	...
	for(i = 0; i<SIZE; i++){
		if(i != c_num){
			possibleGrid[r_num][i] &= ~((1LL)<<(input[r_num][c_num]-1));  
			if(possibleGrid[r_num][i] == 0){
				return -1;
			}
		}
		if(i != r_num){
			possibleGrid[i][c_num] &= ~((1LL)<<(input[r_num][c_num] - 1));
	...
	for(i = mini_row; i < MINIGRIDSIZE+mini_row; i++){
		for(j = mini_column; j < MINIGRIDSIZE+mini_column; j++){
			int up_size0 = 0;
			if((i != r_num) && (j != c_num)){
				possibleGrid[i][j] &= ~((1LL)<<(input[r_num][c_num] - 1));
...
int loneRanger(int** input){
	...
	while(changed){
		...
		for(r = 0; r < SIZE; r++){
			for(c = 0; c < SIZE; c++){
				if(input[r][c] == 0){
					// ll possible_vals = possibleGrid[r][c];
					ll possible_vals = getPossibleValues(input, r, c);
					if(possible_vals == 0){
						return -1;
					}
					int p2 = isPower2(possible_vals);
				...
				if(input[r][c]==0){
					...
					if(pv1==0){
						...
						// // if(found_dummy == 0){
							...
							for(i=0; i<MINIGRIDSIZE; i++){
								for(j=0; j<MINIGRIDSIZE; j++){
									if((i+mini_row)!=r || (j+mini_column)!=c){
										if(input[mini_row+i][mini_column+j]==0){
...
int** solveSudokuRec(int** input){
	if(isValid(input, input)) return input;
	int r_num;
	int c_num;
	int r;
...
int** solveSudoku(int** input){
	...
	// else if((SIZE <= 16) && (thread_count >= 2)) thread_count = thread_count;
	// else if((SIZE > 16) && (thread_count >= 4)) thread_count = thread_count;
